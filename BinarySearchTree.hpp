...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    //correct size condition?
    Node *copied = new Node;
    copied->datum = node->datum;
    copied->left = copy_nodes_impl(node->left);
    copied->right = copy_nodes_impl(node->right);
    return copied;
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    return (node->left && less((max_element_impl(node->left))->datum, node->datum) &&
            check_sorting_invariant_impl(node->left, less)) ||
           (node->right && less(node->datum, (min_element_impl(node->right))->datum) &&
