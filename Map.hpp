...
private:
  ...
  class PairComp {
     ...
     public:
      // from video: overload call operator
      bool operator()(const Pair_type pair1, const Pair_type pair2) const {
          return k_c(pair1.first, pair2.first);
      }
  };
...
public:
  ...
  bool empty() const{
    return bst.empty();
    // utilize the ones you already wrote
  }
  ...
  Value_type& operator[](const Key_type& k){
    if (bst.find({k, Value_type()}) == bst.end()){
      bst.insert({k, Value_type()});
    }
    // return reference to mapped of k 
